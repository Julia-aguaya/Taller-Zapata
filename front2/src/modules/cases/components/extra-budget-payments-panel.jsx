...
export const ExtraBudgetPaymentsPanel = ({ caseId, caseTypeCode, onSaved, onRegisterClientPayment }) => {
  ...
  const canAnnulPayments = hasExceptionalFinancePermission && hasGlobalAdminScope(session);
  const lacksGlobalAnnulmentScope = hasExceptionalFinancePermission && !hasGlobalAdminScope(session);
  const refresh = async () => {
    await Promise.all([
  ...
  return <section className="rounded-3xl border border-border/70 bg-card p-5" aria-labelledby="extra-payment-heading">
    ...
    {lacksGlobalAnnulmentScope ? <p role="alert" className="mt-3 text-sm text-destructive">No tenés alcance administrativo global para anular pagos de trabajos extras.</p> : null}
    {payments.length > 0 && <div className="mt-4 space-y-2" aria-label="Historial de pagos extra">{payments.map((payment) => <div key={payment.id ?? payment.movementId} className="flex items-center justify-between rounded-xl border border-border/60 px-3 py-2 text-sm"><span>{Number(payment.amount) < 0 ? 'Reversión de pago' : `Pago ${currency.format(payment.amount)}`}</span>{canAnnulPayments && Number(payment.amount) > 0 && !reversedOriginalIds.has(payment.id) && <Button variant="outline" size="sm" disabled={annulmentMutation.isPending} onClick={() => annulmentMutation.mutate(payment.movementId)}>Anular pago</Button>}</div>)}</div>}
  </section>;
};
