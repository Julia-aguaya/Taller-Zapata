...
const emptyInsurance = { insuranceCompanyId: '', thirdPartyCompanyId: '', coverageDetail: '', cleasNumber: '', claimNumber: '', processorPersonId: '', inspectorPersonId: '' };
...
export const CleasInsuranceDataSection = ({ caseId, onHydrated, initialCleasNumber = '', onCleasNumberChange, onClaimNumberChange }) => {
  ...
  const mutation = useMutation({
    mutationFn: () => {
      ...
      return saveCleasInsurance(caseId, {
        insuranceCompanyId: Number(draft.insuranceCompanyId),
        coverageDetail: draft.coverageDetail || null,
        thirdPartyCompanyId: draft.thirdPartyCompanyId ? Number(draft.thirdPartyCompanyId) : null,
        cleasNumber: draft.cleasNumber || null,
        claimNumber: draft.claimNumber || null,
        processorPersonId: draft.processorPersonId ? Number(draft.processorPersonId) : null,
        inspectorPersonId: draft.inspectorPersonId ? Number(draft.inspectorPersonId) : null,
      });
    },
  ...
  return <Card className="rounded-3xl border-border/70 p-5">
    ...
    <div className="mt-4 grid gap-x-6 gap-y-3 md:grid-cols-2">
      <Field label="Cía. aseguradora del cliente"><select value={draft.insuranceCompanyId} onChange={(event) => setDraft((current) => ({ ...current, insuranceCompanyId: event.target.value, processorPersonId: '', inspectorPersonId: '' }))} className="h-10 w-full rounded-xl border border-input bg-background px-3 text-sm"><option value="">Seleccionar...</option>{(companiesQuery.data ?? []).map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></Field>
      <Field label="N.º de siniestro"><Input value={draft.claimNumber} onChange={change('claimNumber')} /></Field>
      <Field label="Cía. aseguradora del tercero"><select value={draft.thirdPartyCompanyId} onChange={change('thirdPartyCompanyId')} className="h-10 w-full rounded-xl border border-input bg-background px-3 text-sm"><option value="">Sin informar</option>{(companiesQuery.data ?? []).map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></Field>
