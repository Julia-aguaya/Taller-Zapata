        ...
        <div className="rounded-2xl border border-border/60 bg-card p-5">
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div className="grid flex-1 gap-3 md:grid-cols-3">
              ...
              <div className="space-y-1"><Label className="text-xs" htmlFor="accessory-work-vat">IVA</Label><select id="accessory-work-vat" className="h-10 w-full rounded-xl border border-input bg-muted/60 px-3 text-sm text-muted-foreground" value="21" disabled><option value="21">21% sobre MO</option></select></div>
            </div>
          ...
          {/* Grilla local: estos extras no forman parte de las líneas ni del payload del presupuesto. */}
                ...
                <div key={work.id} className="rounded-2xl border border-border/50 bg-background/50 p-4">
                  {/* Grilla de cada extra: mantiene sus importes aislados del presupuesto principal. */}
                    <div className="grid gap-3 md:grid-cols-[minmax(0,1fr)_180px_auto]">
                     <div className="space-y-1"><Label className="text-xs" htmlFor={`accessory-work-detail-${work.id}`}>Detalle</Label><Input id={`accessory-work-detail-${work.id}`} value={work.detail} onChange={(event) => updateAccessoryUi((current) => ({ ...current, works: current.works.map((entry) => entry.id === work.id ? { ...entry, detail: event.target.value } : entry) }))} /></div>
                     <div className="space-y-1"><Label className="text-xs">Total MO</Label><Input type="number" min="0" step="0.01" value={work.amount} onChange={(event) => updateAccessoryUi((current) => ({ ...current, works: current.works.map((entry) => entry.id === work.id ? { ...entry, amount: event.target.value } : entry) }))} /></div>
          ...
          <div className="mt-4 space-y-1"><Label className="text-xs">Anotaciones de accesorios</Label><Textarea rows={3} value={accessoryUi?.notes ?? ''} onChange={(event) => updateAccessoryUi((current) => ({ ...current, notes: event.target.value }))} /></div>
        </div>
      ) : null}
