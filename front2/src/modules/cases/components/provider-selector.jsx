...
export const ProviderSelector = ({ value, providerId, onChange, placeholder = 'Buscar proveedor...' }) => {
  ...
  const providersQuery = useQuery({
    ...
    enabled: debouncedSearch.length >= 2 && !selected && !providerId,
  });
    ...
    <div className="space-y-2">
      {selectedLabel ? (
        <div className="flex min-h-10 items-center gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 text-sm dark:border-emerald-800 dark:bg-emerald-950">
          ...
          <button type="button" className="ml-auto text-xs text-muted-foreground hover:text-destructive" onClick={clearProvider}>Cambiar</button>
        </div>
      ) : (
        <>
          ...
          {(providersQuery.data ?? []).length > 0 ? (
            <div className="rounded-xl border border-border bg-card p-1 shadow-haze">
              {providersQuery.data.map((provider) => (
                <button key={provider.id} type="button" className="w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-accent" onClick={() => selectProvider(provider)}>
                  <span className="font-medium">{provider.name}</span>
                  {provider.phone ? <span className="ml-2 text-xs text-muted-foreground">{provider.phone}</span> : null}
                </button>
              ))}
            </div>
          ) : debouncedSearch.length >= 2 && !providersQuery.isFetching ? (
            <p className="text-xs text-muted-foreground">No encontramos proveedores. Podés cargar el nombre manualmente abajo.</p>
          ) : null}
        </>
      )}
      <Input value={value || ''} onChange={(event) => { setSelected(null); onChange(providerPayload(null, event.target.value)); }} placeholder="O ingresá el proveedor manualmente" />
    </div>
  );
};
