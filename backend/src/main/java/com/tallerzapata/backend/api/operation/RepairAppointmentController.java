...
public class RepairAppointmentController {
    ...
    @Operation(summary = "Eliminar turno", description = "Elimina un turno que todavía no tiene ingreso asociado")
    @ApiResponse(responseCode = "204", description = "Sin contenido")
    @PreAuthorize("hasAuthority('turno.editar')")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    @DeleteMapping("/api/v1/appointments/{appointmentId}")
    public void delete(@PathVariable Long appointmentId, HttpServletRequest httpRequest) {
        repairAppointmentService.delete(appointmentId, httpRequest);
    }
}
